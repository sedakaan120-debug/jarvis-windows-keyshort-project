...
from actions.windows_utils import get_active_window_rect, get_active_window_title
...
def _screen_permission_message() -> str:
    return (
        "Ekran analizi icin Windows ekran goruntusu alinamadi. "
        "Uygulama yonetici izni gerektiren veya korumali bir pencerede calisiyor olabilir."
    )
...
def _run_helper(mode: str, timeout: int = 20) -> tuple[bool, str]:
    try:
        ...
        return False, f"Pillow ImageGrab kullanilamadi: {exc}"
    ...
    try:
        ...
        output_path = Path(tempfile.mkstemp(prefix="jarvis-screen-", suffix=".png")[1])
        ...
        payload = {
            ...
            "detail": "active_window" if rect else "full_screen",
        }
