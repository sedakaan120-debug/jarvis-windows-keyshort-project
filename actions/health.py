...
# Geriye d�n�k uyumluluk � eski konumlar
_LEGACY_DIRS = [
...
STALE_WARN_MINUTES = 120  # 2 saatten eski veriye uyari
...
def _find_health_file(target_date: date | None = None) -> Path | None:
    """
    En son HealthAutoExport-*.json dosyasini bulur.
    �nce asil klas�r�, bulamazsa legacy konumlari dener.
    """
...
def _resolve_file(target_date: date | None = None) -> Path | None:
    """En g�ncel saglik dosyasini d�nd�r�r. Yeni konum �nce, legacy sonra."""
...
def _age_str(ts: float) -> str:
    ...
    if mins < 2:   return "az �nce"
    if mins < 60:  return f"{int(mins)} dakika �nce"
    hrs = mins / 60
    if hrs < 24:   return f"{hrs:.1f} saat �nce"
    return f"{hrs/24:.1f} g�n �nce"
...
def _parse_hae(raw: dict, target_date: date | None = None) -> dict:
    ...
    def latest_qty(entries: list) -> float | None:
        """Listeden en son qty/value degerini al."""
    ...
    def today_sum(entries: list) -> float:
        """Hedef g�ndeki t�m kayitlari topla."""
        total = 0.0
        for e in entries:
    ...
    # Kan oksijeni: genellikle 0.0�1.0 arasinda gelir, 0�100'e �evir
...
def _load(target_date: date | None = None) -> tuple[dict, float, dict, date | None]:
    """JSON dosyasini y�kle, (data_dict, timestamp, raw_json, hedef_tarih) d�nd�r."""
    source_file = _resolve_file(target_date)
    if not source_file:
        raise FileNotFoundError("Saglik dosyasi bulunamadi.")
...
def _build_health_analysis(raw: dict, data: dict, query: str, target_date: date | None, age: str) -> str:
    ...
    if any(k in _normalize_query(query) for k in ("antren", "antreman", "workout", "egzersiz", "spor", "fitness")):
        if exercise_min >= 10:
            lines.append(f"{period} i�in evet, belirgin bir antrenman/egzersiz kaydi var.")
        elif exercise_min > 0:
            lines.append(f"{period} i�in kisa s�reli hareket kaydi var ama tam bir antrenman kadar g��l� g�r�nm�yor.")
        else:
            lines.append(f"{period} i�in belirgin bir antrenman kaydi g�r�nm�yor.")
    else:
        lines.append(f"{period} saglik analizi hazir.")

    lines.append(f"Egzersiz s�resi: {int(round(exercise_min))} dakika.")
    lines.append(f"Aktif kalori: {int(round(calories))} kcal, adim: {int(round(steps))}, mesafe: {distance:.2f} km.")
    if workout_window:
        lines.append(f"En net aktivite penceresi: {workout_window}.")
    if peak_hr is not None:
        hr_text = f"Pik nabiz yaklasik {int(round(peak_hr))} bpm"
        if avg_hr is not None:
            hr_text += f", g�nl�k ortalama nabiz yaklasik {int(round(avg_hr))} bpm"
        lines.append(hr_text + ".")
    if walking_speed is not None:
        lines.append(f"Y�r�me hizi yaklasik {walking_speed:.1f} km/sa.")

    if exercise_min >= 60 or calories >= 600:
        lines.append("Yorum: y�k olduk�a y�ksek, g�n aktif ge�mis.")
    elif exercise_min >= 30 or calories >= 300:
        lines.append("Yorum: orta seviye, verimli bir aktivite g�n�.")
    elif steps >= 7000:
        lines.append("Yorum: belirgin bir y�r�y�s/aktif yasam g�n�.")
    else:
        lines.append("Yorum: y�k hafif, daha �ok g�nl�k hareket d�zeyinde.")

    recovery_notes = []
    if hrv is not None:
        if hrv >= 70:
            recovery_notes.append(f"HRV {hrv:.1f} ms ile iyi g�r�n�yor")
        elif hrv >= 40:
            recovery_notes.append(f"HRV {hrv:.1f} ms ile orta seviyede")
        else:
            recovery_notes.append(f"HRV {hrv:.1f} ms ile d�s�k tarafta")
    if resting_hr is not None:
        recovery_notes.append(f"dinlenim nabzi {int(round(resting_hr))} bpm")
    elif current_hr is not None:
        recovery_notes.append(f"son nabiz �l��m� {int(round(current_hr))} bpm")
    if recovery_notes:
        lines.append("Toparlanma: " + ", ".join(recovery_notes) + ".")

    if effort is not None:
        lines.append(f"Physical effort skoru yaklasik {effort:.1f}.")

    lines.append(f"[g�ncelleme: {age}]")
    return "\n".join(lines)
...
def _format(data: dict, query: str, age: str) -> str:
    ...
    if any(k in q for k in ("nabiz", "nabiz", "kalp", "heart", "bpm", "hrv")):
        return "\n".join([
            f"Anlik nabiz    : {_v(data, 'heart_rate', ' bpm')}",
            f"Dinlenim nabzi : {_v(data, 'resting_hr', ' bpm')}",
            f"HRV            : {_v(data, 'hrv', ' ms', 1)}",
            f"Y�r�y�s nabzi  : {_v(data, 'walking_hr', ' bpm')}",
            f"[g�ncelleme: {age}]",
        ])

    if any(k in q for k in ("adim", "step", "egzersiz", "exercise", "kalori", "aktivite", "activity", "kardiyo", "stand", "ayakta", "mesafe", "distance", "kat")):
        return "\n".join([
            f"Bug�n adim     : {_v(data, 'steps')}",
            f"Aktif kalori   : {_v(data, 'calories', ' kcal')}",
            f"Bazal kalori   : {_v(data, 'basal_calories', ' kcal')}",
            f"Egzersiz s�resi: {_v(data, 'exercise_min', ' dk')}",
            f"Ayakta saat    : {_v(data, 'stand_hours', ' saat')}",
            f"Ayakta s�re    : {_v(data, 'stand_min', ' dk')}",
            f"Y�r�me mesafesi: {_v(data, 'walking_distance_km', ' km', 2)}",
            f"�ikilan kat    : {_v(data, 'flights_climbed')}",
            f"[g�ncelleme: {age}]",
        ])

    if any(k in q for k in ("y�r�y�s", "yuruyus", "y�r�me", "yurume", "walking", "mobility", "denge", "asimetri", "asymmetry", "hiz", "hiz", "adim uzunlugu", "adim uzunlugu", "step length", "double support")):
        return "\n".join([
            f"Y�r�me hizi    : {_v(data, 'walking_speed', ' km/sa', 1)}",
            f"Adim uzunlugu  : {_v(data, 'walking_step_length_cm', ' cm')}",
            f"Y�r�y�s nabzi  : {_v(data, 'walking_hr', ' bpm')}",
    ...
    if any(k in q for k in ("uyku", "sleep", "uyudum")):
        return "\n".join([
            f"Uyku s�resi    : {_v(data, 'sleep_hours', ' saat', 1)}",
    ...
    if any(k in q for k in ("oksijen", "spo2", "oxygen", "solunum")):
        return "\n".join([
            ...
            f"Solunum hizi   : {_v(data, 'respiratory_rate', ' nefes/dk', 1)}",
            f"[g�ncelleme: {age}]",
        ])

    if any(k in q for k in ("ses", "audio", "kulaklik", "kulaklik", "maruziyet", "g�r�lt�", "gurultu", "desibel", "db", "headphone", "environmental")):
        return "\n".join([
            f"�evresel ses   : {_v(data, 'environment_audio_db', ' dB', 1)}",
            f"Kulaklik sesi  : {_v(data, 'headphone_audio_db', ' dB', 1)}",
            f"[g�ncelleme: {age}]",
        ])

    if any(k in q for k in ("g�n isigi", "gun isigi", "daylight", "g�nes", "gunes")):
        return "\n".join([
            f"G�n isigi      : {_v(data, 'daylight_min', ' dk', 1)}",
    ...
    return "\n".join([
        "-- SAGLIK �ZETI ------------------",
        f"?? Nabiz         : {_v(data, 'heart_rate', ' bpm')}  (din.: {_v(data, 'resting_hr', ' bpm')})",
        f"?? HRV           : {_v(data, 'hrv', ' ms', 1)}",
        f"?? Y�r�y�s nabzi : {_v(data, 'walking_hr', ' bpm')}",
        f"?? Kan oksijeni  : {_v(data, 'blood_oxygen', '%', 1)}",
        f"?? Solunum hizi  : {_v(data, 'respiratory_rate', ' nefes/dk', 1)}",
        f"?? Adim          : {_v(data, 'steps')}",
        f"?? Aktif kalori  : {_v(data, 'calories', ' kcal')}",
        f"? Bazal kalori  : {_v(data, 'basal_calories', ' kcal')}",
...
def get_health_data(query: str = "all") -> str:
    ...
    if not _resolve_file(target_date):
        return (
            "Saglik verisi bulunamadi. "
            "iPhone'da desteklenen bir saglik disa aktarma araci kurup "
            "iCloud Drive > Auto Export > JARVIS klas�r�ne export ayarlaman gerekiyor."
        )

    try:
        data, ts, raw, source_date = _load(target_date)
    except Exception as e:
        return f"Saglik dosyasi okunamadi: {e}"

    if not data:
        return "Saglik dosyasi bos veya taninmayan formatta."
    ...
    if mins_old > STALE_WARN_MINUTES:
        result += f"\n??  Veri {age} g�ncellendi � uygulamanin otomatik export ayarini kontrol et."
...
def get_welcome_health_summary() -> str:
    if not _resolve_file():
        return "Saglik verilerin su anda alinamadi."
    ...
    parts = []
    if heart_rate != "�":
        parts.append(f"Kalp atisin {heart_rate}")
    if exercise_min != "�":
        parts.append(f"bug�n {exercise_min} kardiyo yaptin")
    if steps != "�":
        parts.append(f"{steps} adim attin")
    if distance != "�":
        parts.append(f"{distance} y�r�d�n")
    ...
    mins_old = (time.time() - ts) / 60
    if mins_old > STALE_WARN_MINUTES:
        summary += f" Saglik verisi {age} g�ncellendi."
