...
# ── Font sistemi ─────────────────────────────────────────────────────────────
# Windows ve Türkçe karakterler için güvenli sistem fontları.
FONT_BODY_FAMILY = "Segoe UI"
FONT_DISPLAY_FAMILY = "Segoe UI Semibold"
...
class JarvisUI:
    def __init__(self):
        ...
        # Windows window manager bazen geometry'yi override eder, tekrar zorla.
    ...
    def _build_settings_panel(self):
        ...
        # --- YENI EKLENEN MACROS TABI ---
        self._settings_tab_macros = tk.Canvas(
            self._settings_panel,
            width=96,
            height=28,
            bg="#041111",
        ...
        self._settings_tab_macros.bind("<Button-1>", lambda e: self._set_settings_tab("macros"))
        
        self._settings_body = tk.Frame(self._settings_panel, bg="#041111")
        self._debug_body = tk.Frame(self._settings_panel, bg="#041111")
        self._macros_body = tk.Frame(self._settings_panel, bg="#041111")
        self._build_macros_tab_content()
    ...
    def _build_macros_tab_content(self):
        tk.Label(self._macros_body, text="KEYSHORT ENTEGRASYONU", fg=C_PRI, bg="#041111", font=font_body_bold(10)).place(x=0, y=10)
        tk.Label(self._macros_body, text="Tetikleyici Tuş (Örn: ctrl+shift+v):", fg=C_MID, bg="#041111", font=font_body(9)).place(x=0, y=40)
        self._macro_key_entry = tk.Entry(self._macros_body, bg="#020a0a", fg=C_TEXT, insertbackground=C_TEXT, borderwidth=0, highlightthickness=1, highlightbackground=C_DIM, font=font_body(10))
        self._macro_key_entry.place(x=0, y=60, width=260, height=28)
        tk.Label(self._macros_body, text="Çalıştırılacak Hedef (Dosya / URL):", fg=C_MID, bg="#041111", font=font_body(9)).place(x=0, y=100)
        self._macro_target_entry = tk.Entry(self._macros_body, bg="#020a0a", fg=C_TEXT, insertbackground=C_TEXT, borderwidth=0, highlightthickness=1, highlightbackground=C_DIM, font=font_body(10))
        self._macro_target_entry.place(x=0, y=120, width=200, height=28)
        tk.Button(self._macros_body, text="SEÇ", command=self._select_macro_target, bg="#062020", fg=C_PRI, activebackground=C_PRI, activeforeground=C_BG, borderwidth=0, cursor="hand2", font=font_body_bold(9)).place(x=210, y=120, width=50, height=28)
        tk.Button(self._macros_body, text="MAKROYU KAYDET", command=self._save_macro, bg=C_PRI, fg=C_BG, activebackground="#00ffcc", activeforeground=C_BG, borderwidth=0, cursor="hand2", font=font_body_bold(9)).place(x=0, y=170, width=260, height=32)
    ...
    def _build_macros_tab_content(self):
        ...
        # Kaydet Butonu (Genişliği biraz daralttık)
        tk.Button(self._macros_body, text="MAKROYU KAYDET", command=self._save_macro, bg=C_PRI, fg=C_BG, activebackground="#00ffcc", activeforeground=C_BG, borderwidth=0, cursor="hand2", font=font_body_bold(9)).place(x=0, y=170, width=170, height=32)
        
        # YENİ: Makro Silme Butonu (Kırmızı Tonlarda fiyakalı bir buton)
        tk.Button(self._macros_body, text="MAKROYU SİL", command=self._delete_macro, bg="#3a0d0d", fg="#ff4d4d", activebackground="#ff4d4d", activeforeground="#3a0d0d", borderwidth=0, cursor="hand2", font=font_body_bold(9)).place(x=180, y=170, width=80, height=32)

    def _select_macro_target(self):
        filepath = filedialog.askopenfilename(
            title="Çalıştırılacak Dosyayı Seç",
            filetypes=(("Uygulamalar ve Kısayollar", "*.exe *.lnk *.bat"), ("Tüm Dosyalar", "*.*"))
        )
        if filepath:
            self._macro_target_entry.delete(0, tk.END)
            self._macro_target_entry.insert(0, filepath)

    def _save_macro(self):
        hotkey = self._macro_key_entry.get().strip()
        target = self._macro_target_entry.get().strip()
        if not hotkey or not target:
            self.write_log("ERR: Makro kaydetmek için hem tuş hem de hedef belirtmelisin.")
            return
        ...
        if macro_file.exists():
            try:
                ...
                pass
        macros[hotkey] = target
        macro_file.parent.mkdir(parents=True, exist_ok=True)
        with open(macro_file, "w", encoding="utf-8") as f:
            json.dump(macros, f, indent=4, ensure_ascii=False)
        self.write_log(f"SYS: Yeni makro eklendi. Tuş: [{hotkey}] -> Hedef: {target}")
        self._macro_key_entry.delete(0, tk.END)
        self._macro_target_entry.delete(0, tk.END)

    # YENİ: Silme Motoru Fonksiyonu
    def _delete_macro(self):
        hotkey = self._macro_key_entry.get().strip()
        if not hotkey:
            self.write_log("ERR: Silmek istediğin makronun tetikleyici tuşunu (Örn: ctrl+shift+v) yazmalısın.")
            return
        macro_file = BASE_DIR / "config" / "macros.json"
        if not macro_file.exists():
            self.write_log("ERR: Kayıtlı hiçbir makro bulunamadı.")
            return
        try:
            with open(macro_file, "r", encoding="utf-8") as f:
                macros = json.load(f)
            if hotkey in macros:
                ...
                with open(macro_file, "w", encoding="utf-8") as f:
                    json.dump(macros, f, indent=4, ensure_ascii=False)
                self.write_log(f"SYS: [{hotkey}] makrosu başarıyla hafızadan silindi.")
                self._macro_key_entry.delete(0, tk.END)
                self._macro_target_entry.delete(0, tk.END)
            else:
                self.write_log(f"ERR: Hafızada [{hotkey}] adında bir makro bulunamadı.")
        except Exception as e:
            self.write_log(f"ERR: Makro silinirken hata oluştu: {e}")
    ...
    def _draw_settings_tabs(self):
        for key, canvas, label in (
            ...
            ("macros", self._settings_tab_macros, "MACROS"),
        ):
    ...
    def _draw_mute_button(self):
        ...
        c.delete("all")
        if self.muted:
            col, icon, lbl = C_MUTED, "[M]", " MUTED"
        else:
            col, icon, lbl = C_GREEN, "[MIC]", " LIVE"
        bl = 6
        for bx, by, sx, sy in [(0, 0, 1, 1), (bw, 0, -1, 1),
                                (0, bh, 1, -1), (bw, bh, -1, -1)]:
    ...
    def _place_layout_widgets(self):
        ...
        if self._settings_open:
            ...
            self._settings_tab_macros.place(x=238, y=40) # MACROS butonu eklendi
            
            if self._settings_tab == "debug":
                self._settings_body.place_forget()
                self._macros_body.place_forget()
            ...
            elif self._settings_tab == "macros":
                self._settings_body.place_forget()
                self._debug_body.place_forget()
                self._macros_body.place(x=12, y=76, width=panel_w - 24, height=panel_h - 88)
                self._macros_body.lift() # MACROS penceresi gosteriliyor
            else:
                self._debug_body.place_forget()
                self._macros_body.place_forget()
            ...
            self._settings_tab_macros.place_forget()
            self._settings_body.place_forget()
            self._debug_body.place_forget()
            self._macros_body.place_forget()
